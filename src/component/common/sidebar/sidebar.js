...
const Sidebar = (props) => {
  ...
  useEffect(() => {
    ...
    return (
        <div className="iconsidebar-menu iconbar-mainmenu-close">
          <div className="sidebar custom-scrollbar">
            ...
            <li className={`left-arrow ${layout_type === 'rtl' ? hideLeftArrowRTL ? 'd-none' : 'hideLeftArrowRTL' : hideLeftArrow ? 'd-none' : 'hideLeftArrow'}`}
                    onClick={(configDB.sidebar_type === 'horizontal_sidebar' && layout_type === 'rtl') ? scrollToLeftRTL : scrollToLeft}><i className="fa fa-angle-left"></i>
            </li>
            {
                  MENUITEMS.map((menuItem, i) => 
                      ...
                      <li className={`${menuItem.active ? 'open' : ''}`} key={i}>
                        {menuItem.tag}
                        <Link className="bar-icons" to={`${menuItem.path}`} onClick={() => toggletNavActive(menuItem)}>
                        {menuItem.icon}<span>{menuItem.title}</span></Link>
                           ...
                           <Fragment key={index}>
                            ...
                            {(childrenItem.type === 'link') ?
                                  <li className={childrenItem.active ? 'active' : ''}>
                                    <Link className={childrenItem.active ? 'active' : ''} to={`${process.env.PUBLIC_URL}${childrenItem.path}`} onClick={() => toggletNavActive(childrenItem)}>{childrenItem.title}</Link>
                                  </li>
                                  : ""}
                            </Fragment>
                          )}
                ...
                <li className={`right-arrow ${layout_type === 'rtl' ? hideRightArrowRTL ? 'd-none' : 'hideRightArrowRTL' : hideRightArrow ? 'd-none' : 'hideRightArrow'}`}
                                onClick={(sidebar_type === 'horizontal_sidebar' && layout_type === 'rtl') ? scrollToRightRTL : scrollToRight}><i className="fa fa-angle-right"></i>
                </li>
            </ul> 
          </div>
        </div>
